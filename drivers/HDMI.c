...
static inline bool hdmi_init() {
    ...
    // Only set IRQ handler and start DMA here if NOT deferring to Core 1
    // When deferred, Core 1 will call graphics_init_irq_on_this_core() which
    // will then start the DMA
    if (!g_defer_irq_to_core1) {
        irq_set_exclusive_handler_DMA_core1();
        dma_start_channel_mask((1u << dma_chan_ctrl));
    }
    // If deferred, DMA will be started by graphics_init_irq_on_this_core()
...
void graphics_init_irq_on_this_core(void) {
    ...
    // If we deferred, the DMA wasn't started yet - start it now
    if (g_defer_irq_to_core1) {
        dma_start_channel_mask((1u << dma_chan_ctrl));
    }
