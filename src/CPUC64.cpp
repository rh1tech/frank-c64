...
MOS6510::MOS6510(C64 *c64, uint8_t *Ram, const uint8_t *Basic, const uint8_t *Kernal, const uint8_t *Char, uint8_t *Color)
...
#else
		char buf[64];
		snprintf(buf, sizeof(buf), "C64 crashed at $%04X, press F12 to reset", adr);
		std::string s = buf;
#endif
